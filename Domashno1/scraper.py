...
def get_last_scraped_date(conn):
    with conn.cursor() as cur:
        cur.execute("SELECT MAX(TO_DATE(date, 'DD.MM.YYYY')) FROM stock_prices_final")
        result = cur.fetchone()
...
def insert_data_to_db(conn, stock_data):
    ...
    insert_query = """
        INSERT INTO stock_prices_final (
...
def fetch_issuer_data(args):
    issuer, start_date, end_date = args
    issuer_data = []
    ...
    if start_date == end_date:
        ...
        with Pool(processes=15) as pool:
            for issuer in issuers_data:
                args = [(issuer, *dr) for dr in date_ranges]
                results = pool.map(fetch_issuer_data, args)
